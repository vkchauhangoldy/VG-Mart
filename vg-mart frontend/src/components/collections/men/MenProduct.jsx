...
const MenProducts = () => {
    ...
    const [shirtDatas, setShirtDatas] = useState([]);
    ...
    const getAllMensCollections = async () => {
        try {
            const [response1, response2, response3] = await Promise.all([
                axios.get(`${BASE_URL}/category/mens-shirts`),
                axios.get(`${BASE_URL}/category/mens-watches`),
                axios.get(`${BASE_URL}/category/mens-shoes`)
            ]);

            setShirtDatas(response1.data?.products)
            setWatchDatas(response2.data?.products)
            setShoesDatas(response3.data?.products)
        } catch (error) {
                ...
                isLoading ? <div className="loader"><Loader /></div>
                    : <>
                        <div className="container">
                            <div className=" row justify-content-center pt-5">
                                <div className="col-lg-6 col-xl-7 text-center">
                                    <h3 className="ezy__testimonial1-heading m-3">{"Men's Collections"}</h3>
                        ...
                        <div className="conatiner">
                            <h4 className=" p-3">{"Men's Shirt Collections"}</h4>
                            <div className="products">
                                {
                                    shirtDatas?.myMap((data) => (
                        ...
                        <div className="conatiner">
                            <h4 className=" p-3">{"Men's Watches Collections"}</h4>
                            <div className="products">
                                {
                                    watchDatas?.map((data) => (
                                        <div key={data.id} className="card mb-3" style={{ width: "18rem" }}>
                        ...
                        <div className="conatiner">
                            <h4 className=" p-3">{"Men's Shoes Collections"}</h4>
                            <div className="products">
                                {
                                    shoesDatas?.map((data) => (
                                        <div key={data.id} className="card mb-3" style={{ width: "18rem" }}>
