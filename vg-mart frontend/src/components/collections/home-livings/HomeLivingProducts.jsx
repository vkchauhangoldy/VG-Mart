    ...
    const getAllMensCollections = async () => {
        try {
            const [response1, response2, response3] = await Promise.all([
                axios.get(`${BASE_URL}/category/furniture`),
                axios.get(`${BASE_URL}/category/home-decoration`),
                axios.get(`${BASE_URL}/category/kitchen-accessories`)
            ]);

            setFurniture(response1.data?.products)
            setDecoration(response2.data?.products)
            setAccessories(response3.data?.products)
        } catch (error) {
