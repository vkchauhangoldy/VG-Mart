...
const BeautyProducts = () => {
    ...
    const [makeupDatas, setMakeupDatas] = useState([]);
    const [fragrancesDatas, setFragrancesDatas] = useState([]);
    const [skinDatas, setSkinDatas] = useState([]);

    const getAllMensCollections = async () => {
        try {
            const [response1, response2, response3] = await Promise.all([
                axios.get(`${BASE_URL}/category/beauty`),
                axios.get(`${BASE_URL}/category/fragrances`),
                axios.get(`${BASE_URL}/category/skin-care`)
            ]);

            setMakeupDatas(response1.data?.products)
            setFragrancesDatas(response2.data?.products)
            setSkinDatas(response3.data?.products)
        } catch (error) {
                ...
                isLoading ? <div className="loader"><Loader /></div>
                    : <>
                        <div className="container">
                            <div className=" row justify-content-center pt-5">
                                <div className="col-lg-6 col-xl-7 text-center">
                                    <h3 className="ezy__testimonial1-heading m-3">{"Beauty Collections"}</h3>
                        ...
                        <div className="conatiner">
                            <h4 className=" p-3">{"Makeup Collections"}</h4>
                            <div className="products">
                                {
                                    makeupDatas?.map((data) => (
                        ...
                        <div className="conatiner">
                            <h4 className=" p-3">{"Fragrances Collections"}</h4>
                            <div className="products">
                                {
                                    fragrancesDatas?.map((data) => (
                                        <div key={data.id} className="card mb-3" style={{ width: "18rem" }}>
                                            <div onClick={() => productDetailsHandler(data.id, data)} className="cursor">
                                                <img src={data.thumbnail} className="card-img-top" alt="..." height="200px" />
                                            </div>
                        ...
                        <div className="conatiner">
                            <h4 className=" p-3">{"Skin Care Collections"}</h4>
                            <div className="products">
                                {
                                    skinDatas?.map((data) => (
                                        <div key={data.id} className="card mb-3" style={{ width: "18rem" }}>
